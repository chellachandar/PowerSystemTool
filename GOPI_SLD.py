...
# print(f"Total feeders found: {num_feeders}")
...
sheet = wb.active  # or specify sheet name: wb["Sheet1"]
b6 = sheet["B6"].value
b8 = sheet["B8"].value
b9 = sheet["B9"].value
d6 = sheet["D6"].value
...
feeder_names = list(df.iloc[2, 2:2+num_feeders])  # C3, D3, E3, ...
# print("Feeder types:", feeder_types)
# print("Feeder types:", feeder_names)
...
def get_fontsize(num_feeders):
    """Dynamically scale font size based on feeder count"""
...
def draw_cvt(ax, x, y, label, fs):
    ...
    # cvt secondary
    spacing = 0.25  # horizontal distance between the two C arcs
...
def draw_symbol(ax, x, y, label, fs):
    ...
    # ax.text(x, y-0.5, label, fontsize=fs, ha='center')
...
if b6 in ["Double Main Transfer Bus", "Double Main Bus"]:
    ...
    def get_labels(feeder_num):
        """Generate standard label names for given feeder number"""
        # wb = openpyxl.load_workbook(file_path)
        # sheet = wb.active  # or specify sheet name: wb["Sheet1"]
    ...
    # def draw_feeder_NO_WT(ax, x_offset, y_offset, feeder_num, fs):
    #     L = get_labels(feeder_num)
    ...
    #     ax.plot([x_offset+0.5, x_offset+0.5], [y_offset, y_offset+0.8], color='red', linewidth=0.5)
    #     ax.plot([x_offset+.25, x_offset+0.5], [y_offset-4.5, y_offset-4.5], color='red', linewidth=0.5)
    #     ax.plot([x_offset+0.5, x_offset+0.5], [y_offset-4.5, y_offset-10], color='red', linewidth=0.5)
    #     # ax.plot([x_offset+0.5, x_offset+0.5], [y_offset-7.75, y_offset-10], color='red', linewidth=0.5)
    ...
    for i in range(num_feeders):
        ...
        if f_name.lower() == 'nan':  # handle empty Excel cells
            f_name = ''
        x_pos = x_start + i * gap
        feeder_label = i + 1

        ax.plot([1, num_feeders*3], [10, 10], color='blue', linewidth=0.5)
        ax.text(1.5, 10.5, f"{b12} KV_BUS1", fontsize=fontsize+4,  va='center') # fontweight='bold',
        ...
        # ax.text(2, 15, 'Powergrid corporation of india limited', fontsize=fontsize+25,  va='center')
        ...
        if f_type == 'Line_Bay':
            ...
            print(f"⚠️ Invalid feeder type '{f_type}' for Feeder {feeder_label}. Defaulting to Type 1.")
            draw_feeder1(ax, x_pos, y_start, feeder_label, fontsize)
    ...
    # Combine wrapped lines into final text
        wrapped_text = "\n".join(lines)

    # Adjust Y position based on number of lines
    # y_shift = 1.0 + 0.3 * (len(lines) - 1)
    ...
    ax.text(center_x, 14, 'POWERGRID CORPORATION OF INDIA LTD', fontsize=fontsize+25, va='center', ha='center')  # B8 main title
    ax.text(center_x, 12, b9, fontsize=fontsize+15, va='center', ha='center')  # B9 subtitle
    ...
    # Automatically generate the image path in the same folder as the Excel file
    img_path = os.path.join(
        os.path.dirname(file_path),                     # same folder
        os.path.splitext(os.path.basename(file_path))[0] + "_sld.png"  # same name + _sld
    )
    ...
    img.width = img.width = num_feeders*100*2
    ...
    print(" Diagram successfully replaced in Excel file.")
    plt.show()

# elif b6=="One_and_half_Busbar":
else:
...
# Check for every 3rd pair pattern: (1,3), (4,6), (7,9) ...
            if current_type in ["Line_Bay", "Future_Bay"] and ((i + 1) % 3 == 1 or (i + 1) % 3 == 0):
                bay_num = i + 1

    # Determine the other bay in the pair
                pair_index = i - 2 if (i + 1) % 3 == 0 else i + 2  # 1->3, 4->6, etc.
                pair_index = pair_index if pair_index < len(feeder_types) else None

    # # Only change name if pair exists and both bays have same type
    #             if pair_index is not None:
    #                 pair_type = feeder_types[pair_index]
    #                 if pair_type in ["Line_Bay", "Future_Bay"]:
    #                     if (bay_num - 1) % 3 == 0:      # 1, 4, 7, 10...
    #                         ct_label = f"{b12+feeder_num+1}BCT"
    #                     elif bay_num % 3 == 0:          # 3, 6, 9, 12...
    ...
    #             else:
    #                 ct_label = f"{b12+feeder_num}CT"
    # Check if pair bay exists
                if pair_index is not None:
                    pair_type = feeder_types[pair_index]

        # CASE 1: Both are line-type bays (Line_Bay or Future_Bay)
                    if pair_type in ["Line_Bay", "Future_Bay"]:
                        if (bay_num - 1) % 3 == 0:      # 1,4,7,10,...
                            ct_label = f"{b12+feeder_num+1}BCT"
                        elif bay_num % 3 == 0:          # 3,6,9,12,...
        ...
        # CASE 2: Not both are line-type → still follow offset CT naming
                    else:
                        if (bay_num - 1) % 3 == 0:      # 1,4,7,...
                            ct_label = f"{b12+feeder_num+1}CT"
                        elif bay_num % 3 == 0:          # 3,6,9,...
                            ct_label = f"{b12+feeder_num-1}CT"
                        else:
                            ct_label = f"{b12+feeder_num}CT"
                else:
                    # No valid pair, use offset logic anyway
                    if (bay_num - 1) % 3 == 0:
                        ct_label = f"{b12+feeder_num+1}CT"
                    elif bay_num % 3 == 0:
                        ct_label = f"{b12+feeder_num-1}CT"
    ...
    # For other bay types (non-line/future)
                bay_num = i + 1
                if (bay_num - 1) % 3 == 0:      # 1,4,7,...
                    ct_label = f"{b12+feeder_num+1}CT"
                elif bay_num % 3 == 0:          # 3,6,9,...
...
# ISOLATOR C
    ...
    def middle_common(ax, x_offset, y_offset, feeder_num, fs):
        ...
        # draw_ct(ax, x_offset+0.25, y_offset-3.5-.1, L["ct_lbl"], fs)
    ...
    y_gap = 6.2+.6  # vertical gap between feeders
    x_gap = gap  # horizontal gap from your code

    # Draw Bus lines only once (top of diagram)
    ax.plot([1, num_feeders*2], [20.7,20.7], color='blue', linewidth=0.5)
    # ax.text(1.5, 20.5, 'Bus1', fontsize=fontsize+4, va='center')
    # ax.text(3, 30.5, 'Powergrid corporation of india limited', fontsize=fontsize+25, va='center')
    ax.plot([1, num_feeders*2], [-.2,-.2], color='green', linewidth=0.5)
    # ax.text(1.5, 1, 'Bus2', fontsize=fontsize+4, va='center')
    # ax.plot([1, num_feeders*3], [2.4, 2.4], color='green', linewidth=0.5)
    # ax.text(1.5, 2.4, 'Transfer BUS', fontsize=fontsize+4, va='center')
    ...
    for i in range(num_feeders):
        ...
        col = i // feeders_per_column       # column index
        row = i % feeders_per_column        # row index within column

        x_pos = x_start + col * x_gap
        y_pos = y_start - row * y_gap       # decrement Y for vertical stacking
    ...
    ax.text(center_x, 32, 'POWERGRID CORPORATION OF INDIA LTD', fontsize=fontsize+25, va='center', ha='center')  # B8 main title
    ax.text(center_x, 30, b9, fontsize=fontsize+15, va='center', ha='center')  # B9 subtitle
    ...
    # 0, x_start + i * gap
    # 0, x_start + num_columns * x_gap
    ax.set_ylim(-10,35)  # adjust as per total vertical space
    ax.axis('on')
