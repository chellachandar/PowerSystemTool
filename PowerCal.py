...
/* compact headings */
h1 {font-size:1.35rem !important; padding-bottom:0;}
h3 {font-size:1.0rem !important; padding:0.4rem 0 0.2rem;}
/* metrics sized for phone — big enough to read, small enough not to wrap */
[data-testid="stMetricValue"] {font-size:1.45rem;}
[data-testid="stMetricLabel"] p {font-size:0.82rem; font-weight:600; color:#3c4a63;}
/* formula captions readable on mobile */
[data-testid="stCaptionContainer"] p {font-size:0.78rem; line-height:1.25;}
/* tabs: compact, scrollable */
button[data-baseweb="tab"] {padding:6px 10px;}
button[data-baseweb="tab"] p {font-size:0.85rem; font-weight:600;}
/* input labels */
[data-testid="stNumberInput"] label p {font-size:0.8rem; font-weight:600;}
...
def classify_scr(v):
    if not math.isfinite(v):
        return ("—", "#5c6d85", "Enter valid inputs.")
    if v > 3:
        return ("STRONG", "#177a4c", "Standard control and protection settings expected to perform normally.")
    if v >= 2:
        return ("WEAK", "#a86e0a", "Review control interactions and protection coordination.")
    return ("VERY WEAK", "#bd3b2f", "EMT studies mandatory; consider sync condensers / reinforcement.")
...
def classify_sir(v):
    ...
    if v > 4:
        return ("HIGH", "#bd3b2f", "Short line — watch Zone-1 transient overreach and CVT response.")
    if v >= 0.5:
        return ("MEDIUM", "#177a4c", "Typical line — standard distance settings generally valid.")
    return ("LOW", "#a86e0a", "Long line — load encroachment / power-swing blocking dominate.")


def badge(label, color):
    st.markdown(
        f"<span style='background:{color};color:#fff;font-size:11px;font-weight:700;"
        f"letter-spacing:0.06em;padding:3px 10px;border-radius:20px'>{label}</span>",
        unsafe_allow_html=True,
    )
...
def note(text):
    st.markdown(
        ...
        f"padding:8px 12px;font-size:12.5px;line-height:1.35;color:#132a47;margin-top:2px'>{text}</div>",
        unsafe_allow_html=True,
    )


st.title("⚡ P&C Field Calculator")
st.caption("Indicative — verify against detailed studies and rated data")

tab_tx, tab_ck, tab_fl, tab_sc, tab_q, tab_set = st.tabs(
    ["Transformer", "Circuit", "Fault", "SCR/SIR", "Reactive", "⚙"]
)

# ---------------------------------------------------------------- Transformer
with tab_tx:
    st.session_state.txS = st.number_input("Rating S (MVA)", value=st.session_state.txS, key="txS_in")
    c1, c2 = st.columns(2, gap="small")
    st.session_state.txUp = c1.number_input("Primary U₁ (kV)", value=st.session_state.txUp, key="txUp_in")
    st.session_state.txUs = c2.number_input("Secondary U₂ (kV)", value=st.session_state.txUs, key="txUs_in")
    ...
    with r3:
        st.metric("CT — HV", suggest_ct(Ip, st.session_state.ctMargin))
    with r4:
        st.metric("CT — LV", suggest_ct(Is, st.session_state.ctMargin))
    st.caption(f"Nearest standard ratio ≥ {st.session_state.ctMargin}% full-load · series 100…5000 / 1 A")

    st.subheader("Short-circuit test")
    c1, c2 = st.columns(2, gap="small")
    st.session_state.txZ = c1.number_input("Impedance Z (%)", value=st.session_state.txZ, key="txZ_in")
    st.session_state.txVin = c2.number_input("Injected V (kV)", value=st.session_state.txVin, key="txVin_in")

    b1, b2 = st.columns(2, gap="small")
    if b1.button("Inject LV", use_container_width=True,
                 type="primary" if st.session_state.txInj == "lv" else "secondary"):
        st.session_state.txInj = "lv"
        st.rerun()
    if b2.button("Inject HV", use_container_width=True,
    ...
    with r5:
        st.metric(f"{injName} (injected)", f"{fI(tIinj)} A")
        st.caption("I = Irated × Vinj / (Z% × Urated)")
    with r6:
        st.metric(f"{othName} (shorted)", f"{fI(tIoth)} A")
        st.caption("Reflected by ratio U₁/U₂")
    st.metric("Through-fault, full voltage", f"HV {fI(tScP)} · LV {fI(tScS)} A")
    st.caption("Isc = Irated / (Z%/100) — infinite source")
...
with tab_ck:
    ...
    st.session_state.ckA = c3.number_input("Ampacity each (A)", value=st.session_state.ckA, key="ckA_in")
    st.session_state.ckD = c4.number_input("Bundle derating kd", value=st.session_state.ckD, key="ckD_in",
                                           format="%.2f")
    ...
    r1, r2 = st.columns(2, gap="small")
    with r1:
        st.metric("Phase current", f"{fI(cI)} A")
        st.caption("I = ampacity × n × kd")
    with r2:
        st.metric("Capability", f"{fmt(cS, 0)} MVA")
        st.caption("S = √3 × U × I / 1000")
    note("kd covers mutual heating in the bundle — typical 0.95–1.00; use 1.00 if rated values already include bundling.")

    st.subheader("ACSR reference (indicative)")
...
with tab_fl:
    ...
    c3, c4 = st.columns(2, gap="small")
    st.session_state.fXR = c3.number_input("X/R", value=st.session_state.fXR, key="fXR_in")
    st.session_state.fSb = c4.number_input("Base MVA", value=st.session_state.fSb, key="fSb_in")
    st.session_state.fZ01 = st.number_input("Z₀ / Z₁", value=st.session_state.fZ01, key="fZ01_in")
    ...
    r1, r2 = st.columns(2, gap="small")
    with r1:
        st.metric("Source Zs", f"{fmt(Z, 3)} Ω")
        st.caption("Zs = U² / Sf")
    with r2:
        st.metric("3-ph fault", f"{fmt(If, 1)} kA")
        st.caption("If = Sf / (√3 × U)")
    st.metric("1-ph (SLG) fault", f"{fmt(I1, 1)} kA ({fmt(I1/If*100, 0)}% of 3-ph)")
    st.caption("I1ph = 3×If₃ / (2 + Z₀/Z₁) — bolted, solidly earthed")
    r3, r4, r5 = st.columns(3, gap="small")
    r3.metric("X", f"{fmt(X, 2)} Ω")
    r4.metric("R", f"{fmt(R, 2)} Ω")
    r5.metric("∠Z", f"{fmt(th*180/math.pi, 1)}°")
    st.caption("X = Z·sin(atan X/R) · R = Z·cos(·)")
    st.metric("Per-unit", f"{fmt(Zpu, 4)} pu")
    st.caption("Zpu = Sbase / Sf")

# ---------------------------------------------------------------- SCR / SIR
with tab_sc:
    st.subheader("SCR — HVDC strength")
    c1, c2 = st.columns(2, gap="small")
    st.session_state.sSf = c1.number_input("Fault level PCC (MVA)", value=st.session_state.sSf, key="sSf_in")
    st.session_state.sP = c2.number_input("Pdc (MW)", value=st.session_state.sP, key="sP_in")
    st.session_state.sQ = st.number_input("Filter/shunt Q (Mvar)", value=st.session_state.sQ, key="sQ_in")
    ...
    badge(f"SIR · {sir_label}", sir_color)
...
# ---------------------------------------------------------------- Reactive
with tab_q:
    st.subheader("1 · Shunt reactor — line energisation")
    c1, c2 = st.columns(2, gap="small")
    ...
    st.session_state.lcQkm = c2.number_input("Charging (Mvar/km)", value=st.session_state.lcQkm, key="lcQkm_in",
                                             format="%.2f")
    st.session_state.lcK = st.number_input("Compensation k (%)", value=st.session_state.lcK, key="lcK_in")
    ...
    with r2:
        ...
        st.caption(f"Qr = k × Qc · {split} per end if split")
    note("Typical 400 kV ≈ 0.55–0.60 Mvar/km; compensate 60–80%, split between ends. Never 100% — resonance risk.")

    st.subheader("2 · Voltage correction")
    c1, c2 = st.columns(2, gap="small")
    ...
    st.session_state.vcS = st.number_input("Bus fault level Ssc (MVA)", value=st.session_state.vcS, key="vcS_in")
    ...
    if not math.isfinite(vcQ):
        vc_label, vc_color, vc_note = "—", "#5c6d85", "Enter voltages and bus fault level."
    elif abs(dU) < 0.005:
        vc_label, vc_color, vc_note = "WITHIN BAND", "#177a4c", "Within ±0.5% of target — no plant needed."
    elif vcQ > 0:
        vc_label, vc_color = "INDUCTIVE — REACTOR", "#a86e0a"
        vc_note = f"Overvoltage: absorb ≈{fmt(abs(vcQ), 0)} Mvar (shunt reactor) to pull bus to target."
    else:
        vc_label, vc_color = "CAPACITIVE — CAP/SVC", "#bd3b2f"
        vc_note = f"Undervoltage: inject ≈{fmt(abs(vcQ), 0)} Mvar capacitive (cap bank / SVC / STATCOM)."
